...
|---|---|---|
| **harness-core** | 18 hooks (defense-in-depth). Flagship trio: credential value scrub (PostToolUse) + dangerous bash guard (PreToolUse) + admission-keyword workflow reminder (UserPromptSubmit); plus branch-policy / rotation-propagation / 真田 auto-backup guards, session-context rails, and a Stop/SubagentStop security-review depth gate. | Every Bash call, user prompt, and session stop |
