You are the unattended weekly harness red-team runner on chichibu. Do EXACTLY the following, deterministically — no creative scope:

## 1. Run the audit workflow
Invoke the Workflow tool:
`Workflow({scriptPath: "/home/hrmtz/projects/claude-harness/plugins/harness-magi/workflows/harness_red_team.js"})`
...
If the Workflow call fails entirely, post a failure line to Discord (step 4 format, but say "RUN FAILED: <reason>") and stop.
...
`/home/hrmtz/projects/claude-harness/docs/redteam/AUDIT_<YYYYMMDD>.md`
Do NOT git commit it (leave it as an artifact for human review).

## 3. File ONLY CRITICAL findings as gh issues
For each finding with `severity == "CRITICAL"` (and ONLY CRITICAL — never HIGH/MED/LOW/nit), create one gh issue on hrmtz/claude-harness:
- Write each issue body to a temp file first, then `gh issue create -R hrmtz/claude-harness --title "[red-team][CRITICAL] <title>" --body-file <tmpfile> --label red-team`.
...
Post a single message to the `claude-harness` channel. To avoid the command-line guard, write the message to a temp file and run `discord-bot post claude-harness "$(cat <tmpfile>)"`. Keep the body to COUNTS and SEVERITIES only — do NOT inline individual finding titles (they may contain .env/DSN literals). Format:
