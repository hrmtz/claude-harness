#!/usr/bin/env python3
"""Regression test: the nightly cron must survive disposal of its caller worktree."""
...
INSTALLER = ROOT / "scripts" / "install_hook_wiring_drift_cron.sh"
RUNNER = ROOT / "scripts" / "run_hook_wiring_drift_cron.sh"
...
class HookWiringCronInstallerTest(unittest.TestCase):
    def test_linked_worktree_installs_primary_checkout_path_once(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            marked = [row for row in rows if MARKER in row]
