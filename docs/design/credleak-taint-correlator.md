...
1. **SOURCES** — where a secret may enter agent-reachable context: a denied `Read` attempt against a classified file; a denied legacy credential-read ACK attempt; a Bash command string containing `sops exec-env` / `sops edit`.
