...
### 01:33 ジュブゼン — 訂正の訂正。**まだ確定していない**

01:33 に「欠落していない、解消」と訂正したが、**その訂正が早すぎた可能性がある**。mz-orch が同じ事象を `active patient defect` と呼んでいるため、事実の粒度を上げさせている。

確定している事実:
- **web サイト**: jubezen ¥132,000 + 新宿院モニター ¥99,000 の**両方が表示されている**
- **live /chat (HTTP 200)**: モニター価格が**無いと応答する**。juvgen の外科併用 ¥79,200 は正しく返る
- 原因は D1 でも cache でもなく **tracked な `_gap_procedures` overlay** (`6f4df3d`、2026-06-24 17:43 JST 導入)

**未確定なのは「これが患者にとって欠落か」**。web を見れば分かるが、**chat しか使わない患者には届かない**。「モニター価格は未掲載」という文言は、患者には「そんな価格は無い」と読める。

mz-orch に確認させていること: chat 利用者が 99,000 円の存在を知れるか / 同じクリニックの 2 窓口で価格情報が食い違うことが医療広告上問題か / 導入から 1 ヶ月間に chat 経由でジュブゼンを問い合わせた**件数** (内容は不要)。

これが「chat 利用者には欠落」と確定したら、clinic への質問は「どう掲載するか」ではなく **「1 ヶ月間 chat でモニター価格が出ていなかったが意図通りか」** に変わる。緊急度が違う。

**誰も価格を変更していない。削除禁止も維持。**
