...
| **harness-craft** | [obra/superpowers](https://github.com/obra/superpowers) (MIT) から蒸留した behavioral craft skill 集: `skill-tdd` (pressure test による RED-GREEN-REFACTOR skill 作成、structural-first 入口ゲート付き)、`atomized-briefing` (formation worker / subagent dispatch 向け 2-5 分粒度 context-free plan)、`root-cause-debugging` (Iron Law + 3 連続 fix 失敗で dual-magi-review へ escalation)、加えて harness オリジナルの `calibrated-verification` (検証器には既知陽性 + 陰性対照を同梱し母数を必ず出力する; [#281](https://github.com/hrmtz/claude-harness/issues/281))。superpowers は教え、harness は強制する — 本 plugin は明示的 behavioral 別館で、強制側に配線済み | SKILL.md 作成時、formation worker / subagent plan の briefing 時、デバッグ全般、検証スクリプトを書く/信じる時 |
