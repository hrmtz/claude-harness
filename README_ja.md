...
- ✅ `harness-formation` — [hrmtz/njslyr7](https://github.com/hrmtz/njslyr7) から蒸留・内蔵。claude + codex worker 対応、session-scoped mailbox identity
- ✅ `harness-kimi` — Kimi Code CLI 向け behavioral + 定期 scrub 移植; 詳細は [plugins/harness-kimi/README.md](./plugins/harness-kimi/README.md)
- ⏳ `harness-grok` — Grok CLI native-hook 移植; Phase 1 実装済 (lib.sh 正規化 / installer / overlay)、Grok Verifier の E2E sign-off 待ち ([docs/grok_hooks.md](./docs/grok_hooks.md) 参照)
