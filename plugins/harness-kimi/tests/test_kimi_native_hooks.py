...
class TestInstaller(unittest.TestCase):
    ...
    def test_install_migrates_legacy_duplicates_and_preserves_user_hook(self):
        self.assertEqual(self._run(INSTALLER).returncode, 0)
        generated = self.config.read_text()
        begin = generated.index(self.MARK_BEGIN)
        marker_end = generated.index(self.MARK_END) + len(self.MARK_END)
        block = generated[begin:marker_end]
        inner = block.split("\n", 1)[1].rsplit(self.MARK_END, 1)[0]
        prefix = generated[:begin]
        user_hook = (
            "[[hooks]]\n"
            "event = 'Notification'\n"
            "command = 'printf user-owned-kimi-hook'\n"
            "timeout = 5\n"
        )
        self.config.write_text(prefix + inner + block + "\n" + user_hook)
        ...
        owned = [
            item
            for item in hooks
            if "/plugins/harness-" in item.get("command", "")
        ]
        credential = [
            item
            for item in owned
            if "credential_file_read_guard.sh" in item["command"]
        ]
        self.assertEqual(len(owned), 16)
        self.assertEqual(len(credential), 2)
        self.assertIn("user-owned-kimi-hook", text)
        first = self.config.read_bytes()
        self.assertEqual(self._run(INSTALLER).returncode, 0)
        self.assertEqual(self.config.read_bytes(), first)

    def test_ambiguous_markers_fail_without_changing_config(self):
        before = (
            'default_model = "kimi-code/k3"\n'
            "# >>> harness-kimi hooks "
        ...
        self.assertIn("expected zero or one ordered", result.stderr)
        self.assertEqual(self.config.read_bytes(), before)

    def test_install_preserves_symlink_and_target_mode(self):
        target = self.tmp / "dotfiles-kimi-config.toml"
        target.write_text('default_model = "symlinked-setting"\n')
        target.chmod(0o640)
        self.config.unlink()
        ...
        self.assertIn("symlinked-setting", target.read_text())
        self.assertIn(self.MARK_BEGIN, target.read_text())
    ...
    def test_dispatcher_trailing_args_do_not_enter_hook_name(self):
        ...
        shutil.copy2(
            REPO / "scripts" / "lib" / "render_kimi_hooks.py",
            lib / "render_kimi_hooks.py",
        )
