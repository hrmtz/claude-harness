...
> formation commit `ca8f1cc` で §1〜§6 を fold-in 完了 (`bin/formation`,
...
|---|---|---|
| **mailbox auto-inject relay daemon** | `scripts/mailbox_relay.sh` (commit `da7a18f0`) | SKILL.md `## Prerequisites` + `formation spawn` の auto-start |
...
user コメント引用 (`feedback_mailbox_relay.md`):
> 「formation の通信機構の改善が必要。mailbox 透過後に awk が入らないので俺が催促しないと読まれない。」

### 提案 v0.2.0

`scripts/mailbox_relay.sh` (inotifywait-driven、debounce 1s) を **prerequisite + auto-start** に組込:
...
4. **Mailbox relay daemon active for each worker pane** (`formation spawn` で auto-start)。
   `inotifywait` で `~/.formation/mailbox/log.jsonl` を監視、新 line の `to` field が
   ...
   詳細: `feedback_mailbox_relay.md` + `scripts/mailbox_relay.sh`
```
...
nohup bash scripts/mailbox_relay.sh <worker_id> <pane_name> \
...
kill $(cat ~/.formation/formation/<id>.relay_pid 2>/dev/null) 2>/dev/null
rm -f ~/.formation/formation/<id>.relay_pid
...
- **scripts/mailbox_relay.sh の項は project-local、global skill にするには ~/.claude/skills/formation/scripts/ に内蔵化必要**
...
### Mailbox discipline
- **Idle 時に必ず `tail -5 ~/.formation/mailbox/log.jsonl` で新着確認**。
...
- **Mailbox に msg 着いてるのに worker が読まない**: relay daemon 落ちてる可能性。
  `ps aux | grep mailbox_relay` で確認、無ければ:
  ```bash
  nohup bash scripts/mailbox_relay.sh <worker_id> <pane_name> \
    > /tmp/relay_<worker_id>.log 2>&1 &
  ```
...
2. **near-term (1 週間以内)**: `## Prerequisites` に relay daemon 必須化 + `formation spawn` auto-start
   - `scripts/mailbox_relay.sh` を `~/.claude/skills/formation/scripts/` に内蔵移動
...
- 関連 commits (predecessor project 側で生まれた前駆 work):
  - predecessor project `da7a18f0` (mailbox relay daemon の最初の実装、scripts/mailbox_relay.sh)
  - predecessor project `5db8a7f3` (double Enter fix)
  - formation `4b98fe2` / `bbb0c4e` で formation lib に取り込み済 (上記 predecessor 側 commit と等価)
- worker memory namespace の実例: `~/.claude/projects/<project-slug>/memory/formation/<worker-id>/` (5 件、Day 17-18 蓄積)
- root memory: `feedback_mailbox_relay.md` / `feedback_host_death_recovery.md` / `user_orchestration_skill_evolution.md`
- 親 audit doc: `docs/MEMORY_AUDIT_RESULT_20260425.md` Phase 5 (本 doc 起案と同 session)
- ritual doc: `docs/MEMORY_SKILLS_AUDIT_RITUAL.md`

> **Note on doc location**: 本 doc は 2026-04-25 起案時点では PRS-LLM-dev/docs に置かれていたが、
> v0.2.0 実装完了後に formation docs (`docs/FORMATION_SKILL_V0_2_0_PROPOSAL.md`) に移動した。
> 上記 reference は履歴メモから移植したもの。公開版では個人 project 名と絶対 path を一般化している。
