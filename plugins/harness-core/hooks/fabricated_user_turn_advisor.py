...
The detector intentionally covers four narrow, high-confidence shapes in the
...
Detection blocks the current Stop once and quarantines every later tool use for
the rest of that session. Each operator approval covers only that tool call.
Malformed payloads, unreadable transcripts, and uncertain record shapes remain
silent success.
